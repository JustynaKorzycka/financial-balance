...
$thisMonthIncome = $db->prepare("SELECT incomes.amount, incomes.date_of_income, incomes.income_comment, inc.name FROM incomes JOIN incomes_category_assigned_to_users AS inc ON incomes.date_of_income>='$firstDay' AND incomes.date_of_income<='$thisDate' AND incomes.user_id='$userId' AND incomes.income_category_assigned_to_user_id=inc.id");

$thisMonthIncome->execute();
$thisMonthBilnasIncome = $thisMonthIncome->fetchAll();

$thisMonthExpenses = $db->prepare("SELECT expenses.amount, expenses.date_of_expense, expenses.expense_comment, expen.name, met.name2 FROM expenses JOIN expenses_category_assigned_to_users AS expen ON expenses.date_of_expense>='$firstDay'
JOIN payment_methods_assigned_to_users AS met ON expenses.payment_method_assigned_to_user_id=met.id
AND expenses.date_of_expense<='$thisDate' AND expenses.user_id='$userId' AND expenses.expense_category_assigned_to_user_id=expen.id");
...
foreach($thisMonthBilnasExpense as $expensCat)
{ 
        ...
        <div class = "row">
            <div class = "col-md-10 col-xs-12 col-md-offset-1 mainBilans">
                <div class = "bilansBlank">
                    <h3>Przegląd bilansu z bieżącego miesiąca</h3>
                    ...
                    <div class="table-responsive col-xs-12 col-md-10 col-md-offset-1 col-xs-offset-0  bilansTable">
                        ...
                        <div class = "col-md-5 col-xs-10 col-xs-offset-1 col-md-offset-1">
                            ...
                            <?php
                                ...
                                if($totalAmount > 0)
                                {
                                    echo "<h2>Brawo!</h2><p> W tym miesiącu oszczędziłeś ".$totalAmount." zł</p>";
                                }
                                else if($totalAmount == 0)
                                {
                                    echo "<h2>Nieźle!</h2><p>W tym miesiącu wyszedłeś na czysto</p>";
                                }
                                else 
                                {
                                    $totalAmount = $totalAmount * (-1);
                                    echo "<h2>Czas zacząć oszczędzać</h2><p>w tym miesiącu wydałeś za dużo o ".$totalAmount." zł</p>";
                                }
                            ?> 
                            </div>
                        </div>
